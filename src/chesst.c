...
//------------------------------------------------------------------------------------
// Infinite loop looking at tester commands received via stdin
//------------------------------------------------------------------------------------
...
int main(int argc, char* argv[])
{
    ...
    while (1) {
        ...
        if (prev_state != game_state) {
            if (game_state <= MAT_GS) send_str_va( "move %s\n", engine_move_str ); // send the move
            else if (game_state == LOST_GS)
                send_str( (play & 1) ? "1-0 {White mates}\n" : "0-1 {Black mates}\n");
        ...
        else if (!strcmp(cmd, "stm"))      set_next_play_time( atoi(arg)); // special tester: en millisecondes
        else if (!strcmp(cmd, "getboard")) {                               // special tester: send the resulting board
            for (int i = 0; i < 64; i++) board[i] = get_piece( i/8, i%8);
            send_str_va("board %s", board);
        }
        else if (!strcmp(cmd, "geteval")) {                               // special tester: send the board eval
            itoa(game_eval, board, 10);
            send_str_va("eval %s\n", board);
        }
